...
## Hypotheses
1) Interannual variability in
 aboveground biomass is greater on uplands than
 on lowlands, and greater in burned than on unburned
 sites  
 2) aboveground biomass in upland sites
 with shallow soils is more strongly related to precipitation than in lowland sites 
...
3) The addition of soil moisture data strengthens the relationship between
 aboveground biomass and meteorological variables.
 
4) Relationships between aboveground biomass and soil moisture in annually
 burned sites are stronger than in unburned sites
 because light or temperature may be the most important
 limiting resources to NPP in unburned prairie
 
5) Interannual variability
 in forb biomass is more strongly related to precipitation compared to the dominant grasses. 
...
to compare productivity among the three burn treatments using each sample as a replicate.

First you'll need a summary table containing the average and standard error of productivity
across all sites for each burn treament. Standard error is calculated as `sd(total_prod)/sqrt(n())`.
...
Use an ANOVA to determine whether the three treatments are statistically different.
...
The treatments are statistically different but burning doesn't explain a lot of the variation among sites. Take a look at the overall variation with a boxplot. 
